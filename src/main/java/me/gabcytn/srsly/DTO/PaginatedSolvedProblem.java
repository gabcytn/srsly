...
public class PaginatedSolvedProblem {
  ...
  public PaginatedSolvedProblem(Page<SolvedProblem> pagedSolvedProblem) {
    List<SolvedProblem> data = pagedSolvedProblem.getContent();
    List<SolvedProblemDto> content = new ArrayList<>();
    for (SolvedProblem problem : data) {
      content.add(problem.toDto());
    }
